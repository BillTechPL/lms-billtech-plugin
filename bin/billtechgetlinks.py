...
def getShortCashLinkByCustomer(cursor, customer_id):
    cursor.execute(
    "select bpl.short_link from billtech_payment_links bpl \
    where customer_id = %s order by id desc limit 1;" % customer_id)
...
def createSingleButton(cursor, customer_id, document_id):
    """
        ...
        :return: Kod html przycisku "Opłać teraz"/"Spłać saldo" w zależności od wartości parametrów
    """
